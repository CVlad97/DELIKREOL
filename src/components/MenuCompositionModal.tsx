...
function ChoiceGroup({ title, choices, required, selected, onToggle }: {
  ...
  if (choices.length === 0 || required === 0) return null;
  return (
    <fieldset className="space-y-3">
      <legend className="font-black text-foreground">{title} <span className="text-sm font-medium text-muted-foreground">— {required} choix</span></legend>
...
export function MenuCompositionModal({ productName, options, onCancel, onConfirm }: Props) {
  ...
  const valid = useMemo(() => sides.length === options.included_side_count && drinks.length === options.included_drink_count, [drinks.length, options.included_drink_count, options.included_side_count, sides.length]);
  const toggle = (value: string, selected: string[], setSelected: (next: string[]) => void, limit: number) => {
    ...
    else if (selected.length < limit) setSelected([...selected, value]);
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="menu-composition-title">
      <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7">
        ...
        {!valid && <p className="mt-5 rounded-xl bg-secondary/10 px-4 py-3 text-sm font-semibold text-secondary">Sélectionnez exactement les choix inclus pour continuer.</p>}
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button type="button" onClick={onCancel} className="min-h-12 rounded-xl border border-border font-bold">Annuler</button>
          <button type="button" disabled={!valid} onClick={() => onConfirm({ sides, drinks })} className="min-h-12 rounded-xl bg-primary px-4 font-black text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">Ajouter au panier</button>
        </div>
      </div>
    </div>
  );
