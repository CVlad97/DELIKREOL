...
export function ClientHomePage({ onSelectMode, onShowGuide, onOpenDemo, onShowLegal, demoMode = false, liteMode = false }: ClientHomePageProps) {
 ...
 const lines = ['Type: Devis entreprise',
 ...
 <a href={`${baseUrl}#commander`} className="hover:text-success">Commander</a>
 <a href={`${baseUrl}#planifier`} className="hover:text-success">Planifier</a>
 <a href={`${baseUrl}#entreprise`} className="hover:text-success">Entreprise</a>
 <a href={`${baseUrl}#partenaires`} className="hover:text-success">Partenaires</a>
 <a href={`${baseUrl}?order-status=1`} className="hover:text-success">Suivi</a>
 </nav>
 <div className="flex items-center gap-3">
 <a
 href={whatsappLink}
 ...
 <a href={`${baseUrl}#partenaires`} className="hover:text-success">Partenaires</a>
 <a href={`${baseUrl}?order-status=1`} className="hover:text-success">Suivi</a>
 <a href={whatsappLink} className="inline-flex items-center gap-2 text-success">
 WhatsApp direct
 <ArrowRight className="w-4 h-4" />
 </a>
 </div>
 ...
 <div>
 <p className="text-xs uppercase tracking-[0.2em] text-success">Checkout local</p>
 ...
 <div className="flex items-center gap-3">
 <div className="text-sm font-black text-success">{item.price.toFixed(2)} €</div>
 <button
 ...
 <div className="text-xs uppercase tracking-[0.2em] text-success">Recapitulatif</div>
 ...
 className="rounded-full border border-emerald-400/40 px-4 py-2 font-semibold text-emerald-200 hover:border-success/30 transition-colors"
 >
 Vérifier ma zone
 </button>
 {geoStatus !=='idle' && (
 ...
 <a href={whatsappLink} className="ml-2 text-success underline underline-offset-4">
 {whatsappNumber}
 </a>
 </div>
 </div>
 ...
 {profile?.planifiable && <span className="rounded-full border border-emerald-500/40 px-2 py-1 text-success">Planifiable</span>}
 {profile?.enterprise && <span className="rounded-full border border-amber-500/40 px-2 py-1 text-amber-300">Entreprise</span>}
 </div>
 <div className="mt-4 text-sm font-semibold text-success">Voir le menu</div>
 </button>
 )})}
 </div>
 </section>
 ...
 <div className="p-8">
 <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.2em] text-success">
 ...
 <span className="font-black text-success">{product.price.toFixed(2)} €</span>
 </div>
 ))}
 </div>
 )}
 ...
 <p className="mt-2 text-sm text-success">{partner.offer}</p>
 ...
 {partner.planifiable && <span className="rounded-full border border-emerald-500/40 px-2 py-1 text-success">Planifiable</span>}
 {partner.enterprise && <span className="rounded-full border border-amber-500/40 px-2 py-1 text-amber-300">Entreprise</span>}
 </div>
 <div className="mt-3 text-xs text-success">{partner.promise} · {partner.eta}</div>
 <button
 onClick={() => openVendorMenu(partner.name)}
 className="mt-4 inline-flex items-center gap-2 text-success font-semibold"
 >
 ...
 className="ml-auto inline-flex items-center gap-2 rounded-full border border-emerald-500/50 px-4 py-2 text-success hover:border-emerald-400"
 >
 Activer ma fidelite
 <ArrowRight className="w-4 h-4" />
 </a>
