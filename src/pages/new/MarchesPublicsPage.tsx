      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-foreground mb-3">
            Appels d'offre <span className="text-primary">publics</span>
          </h1>
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          <select value={filterType} onChange={e => setFilterType(e.target.value)}
            className="px-4 py-2 rounded-xl border border-input text-sm bg-white outline-none focus:border-ring">
          ...
          <span className="text-sm text-muted-foreground self-center ml-auto">{marches.length} marché{marches.length > 1 ? 's' : ''}</span>
        </div>
        ...
        <div className="space-y-4">
          {marches.length === 0 ? (
            ...
            <div key={m.id} className="bg-white rounded-2xl border border-input hover:border-primary/30 shadow-sm hover:shadow-md transition-all p-6">
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-muted-foreground">{TYPE_LABELS[m.type]}</span>
                    <span className="text-xs text-muted-foreground">{m.source}</span>
                  </div>
                  <h3 className="text-lg font-bold text-foreground mb-1">{m.titre}</h3>
                  <p className="text-sm text-muted-foreground mb-3">{m.objet}</p>
                ...
                <div className="flex flex-col gap-2 lg:items-end flex-shrink-0">
                  <a href={m.url} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary/90 text-white font-bold rounded-xl transition-all text-sm">
        ...
        <section className="mt-12 bg-secondary/10 rounded-3xl p-8 border border-secondary/20">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {SOURCES_OFFICIELLES.map(s => (
              <div key={s.nom} className="bg-white rounded-xl p-4 border border-secondary/20">
                <p className="font-bold text-sm text-foreground mb-1">{s.nom}</p>
                <p className="text-xs text-muted-foreground mb-2">{s.description}</p>
                {s.url && <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline font-semibold">Accéder →</a>}
              </div>
            ))}
          </div>
        </section>
        ...
        {/* CTA traiteur */}
        <section className="mt-8 text-center bg-gradient-to-br from-primary to-secondary rounded-3xl p-10 text-white">
