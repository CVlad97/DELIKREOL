...
const DEFAULT_PARTNERS = ["Snack Savè Peyi'A",'Sweet Family Traiteur Orianne','Les Délices de Ninice',"Saveurs d'Afrique","Coco's Food",'Gouté Mwen','Traiteurs de Mada',
];
...
export default function PartnerTerminalPage() {
 ...
 <p className="text-xs font-black uppercase tracking-[0.28em] text-primary/60">MVP sécurisé ce soir</p>
 ...
 <div className="flex items-center gap-3"><ShieldCheck className="text-success" /><span className="font-black">PCI safe</span></div>
 <p className="mt-2 text-sm text-muted-foreground">Aucune donnée carte stockée. 2FA et rapprochement Qonto à activer côté comptes PSP/Qonto.</p>
 </div>
 </div>
 </div>

 <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_.9fr]">
 <section className="rounded-[2rem] border border-primary/20 bg-white p-5 shadow-sm sm:p-6">
 <h2 className="text-2xl font-black">Créer une vente comptoir</h2>
 <div className="mt-5 grid gap-4 sm:grid-cols-2">
 <label className="block"><span className="text-sm font-black">Partenaire</span><select value={partnerName} onChange={(event) => setPartnerName(event.target.value)} className="mt-2 w-full rounded-2xl border border-primary/20 bg-[#fffaf3] px-4 py-3 text-sm">{DEFAULT_PARTNERS.map((name) => <option key={name}>{name}</option>)}</select></label>
 <label className="block"><span className="text-sm font-black">Client</span><input value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Client comptoir" className="mt-2 w-full rounded-2xl border border-primary/20 bg-[#fffaf3] px-4 py-3 text-sm" /></label>
 <label className="block"><span className="text-sm font-black">Téléphone WhatsApp</span><input value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} placeholder="0696 XX XX XX" className="mt-2 w-full rounded-2xl border border-primary/20 bg-[#fffaf3] px-4 py-3 text-sm" /></label>
 <label className="block"><span className="text-sm font-black">Email facture</span><input value={customerEmail} onChange={(event) => setCustomerEmail(event.target.value)} placeholder="client@email.com" className="mt-2 w-full rounded-2xl border border-primary/20 bg-[#fffaf3] px-4 py-3 text-sm" /></label>
 ...
 return <button key={option.id} type="button" onClick={() => setPaymentProvider(option.id as TerminalDraft['payment_provider'])} className={`rounded-2xl border p-4 text-left transition ${paymentProvider === option.id ?'border-primary bg-primary/[0.05]' :'border-primary/20 bg-white'}`}><Icon className="mb-3 text-primary" /><span className="block font-black">{option.label}</span><span className="text-xs text-[#6f5b4b]">{option.text}</span></button>;
 })}
 </div>

 <div className="mt-6 flex items-center justify-between rounded-3xl bg-[#24140d] p-5 text-white">
 <span className="text-sm font-black uppercase tracking-[0.18em] text-primary/60">Total</span>
 <span className="text-3xl font-black">{money(total)}</span>
 </div>
 <button type="button" disabled={items.length === 0 || total <= 0} onClick={createDraft} className="mt-5 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-4 font-black text-white disabled:bg-slate-300">
 Générer facture + suivi <ArrowRight size={18} />
 </button>
 </section>

 <section className="space-y-5">
 <div className="rounded-[2rem] border border-primary/20 bg-white p-5 shadow-sm">
 <h2 className="text-xl font-black">Chaîne sécurisée</h2>
 <div className="mt-4 space-y-3 text-sm">
 {SECURITY_STEPS.map(({ title, text, Icon }) => <div key={title} className="flex gap-3 rounded-2xl bg-[#fff8ef] p-3"><Icon className="mt-1 h-5 w-5 text-primary" /><div><p className="font-black">{title}</p><p className="text-[#6f5b4b]">{text}</p></div></div>)}
 </div>
 </div>

 {createdOrder && (
 <div className="rounded-[2rem] border border-success/20 bg-success/[0.15] p-5 shadow-sm">
 <div className="flex items-center gap-3"><CheckCircle2 className="text-success" /><h2 className="text-xl font-black text-success">Vente créée</h2></div>
 <p className="mt-3 font-mono text-lg font-black">{createdOrder.order_number}</p>
 <p className="mt-1 text-sm text-success">Suivi et facture prêts. Statut payé à confirmer manuellement.</p>
 ...
 <a href={mailInvoiceUrl} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-success/30 px-4 py-3 text-sm font-black text-success"><Mail size={16} /> Email</a>
 <button type="button" onClick={() => navigator.clipboard?.writeText(invoiceText)} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-success/30 px-4 py-3 text-sm font-black text-success"><FileText size={16} /> Copier facture</button>
 <button type="button" onClick={() => setCreatedOrder({ ...createdOrder, payment_status:'paid_external', payment_note:'Marqué comme payé manuellement.' })} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-success/30 px-4 py-3 text-sm font-black text-success"><CheckCircle2 size={16} /> Marquer comme payé manuellement</button>
 </div>
 </div>
 )}
 </section>
