    ...
    <Layout>
      <section className="min-h-[70vh] bg-gradient-to-br from-primary/[0.08] via-secondary/8 to-success/10 px-4 py-10">
        <div className="mx-auto max-w-md rounded-[2rem] border border-primary/20 bg-white p-6 shadow-xl sm:p-8">
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (!googleAuthEnabled) {
                setStatus('error');
                setMessage('Connexion Google en cours de configuration. Utilise le lien email pour accéder à ton espace.');
                return;
              }
              setStatus('loading');
              setMessage('');
          ...
          {!googleAuthEnabled && !isDemoMode && (
            <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold leading-5 text-amber-800">
              Google est désactivé côté production pour éviter une erreur OAuth. Le lien email reste le mode d’accès sécurisé.
            </p>
          )}
