...
const pilotKpis = ['Taux de commande complete avec adresse et contact','Temps de qualification ops','Taux de confirmation vendeur','Delai preparation + livraison','Marge nette par mission','Taux incident / litige',
];
...
export function InvestorOpsPage() {
 ...
 <header className="border-b border-primary/20 bg-[#fff8ec]/95">
 ...
 <div>
 <p className="text-xs font-black uppercase tracking-[0.22em] text-[hsl(var(--primary))]">Investisseur / operations</p>
 <p className="text-lg font-black">DELIKREOL</p>
 </div>
 </a>
 <div className="hidden items-center gap-2 sm:flex">
 <a href={`${baseUrl}#catalogue`} className="rounded-full border border-primary/20 bg-white px-4 py-2 text-sm font-black text-[#7c2d12]">
 Voir catalogue
 </a>
 <a href={`${baseUrl}#partenaires`} className="rounded-full bg-[hsl(var(--primary))] px-4 py-2 text-sm font-black text-white">
 Recruter partenaire
 </a>
 </div>
 </div>
 ...
 <div>
 <span className="inline-flex rounded-full border border-primary/20 bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.2em] text-[#7c2d12]">
 Martinique pilote
 </span>
 ...
 <a href={baseUrl} className="inline-flex items-center justify-center gap-2 rounded-full border border-primary/20 bg-white px-6 py-4 text-sm font-black text-[#7c2d12]">
 Retour accueil
 </a>
 </div>
 </div>

 <div id="readiness" className="rounded-[2rem] border border-primary/20 bg-white p-5 shadow-elegant">
 <div className="flex items-start justify-between gap-4">
 <div>
 <p className="text-xs font-black uppercase tracking-[0.22em] text-[hsl(var(--primary))]">Readiness investisseur</p>
 <h2 className="mt-2 text-3xl font-black">{readiness ? `${readiness.score}/100` :'Calcul en cours'}</h2>
 </div>
 <ShieldCheck className="h-10 w-10 text-success" />
 </div>
 <div className="mt-4 h-3 overflow-hidden rounded-full bg-primary/[0.15]">
 <div
 ...
 <p className="text-xs font-black uppercase tracking-[0.18em] text-muted-foreground">Points a traiter avant pitch</p>
 ...
 <div key={signal.label} className="rounded-[1.5rem] border border-primary/20 bg-white p-5 shadow-soft">
 ...
 <div className="mt-8 overflow-hidden rounded-[2rem] border border-primary/20 bg-white shadow-soft">
 <div className="grid grid-cols-1 border-b border-primary/20 bg-[#24170f] text-sm font-black uppercase tracking-[0.14em] text-orange-100 md:grid-cols-[1fr_1fr_1fr_1.2fr]">
 ...
 <div key={item.segment} className="grid grid-cols-1 border-b border-primary/20 last:border-b-0 md:grid-cols-[1fr_1fr_1fr_1.2fr]">
 ...
 <div key={step.id} className="rounded-[1.5rem] border border-primary/20 bg-white p-5 shadow-soft">
 <div className="flex items-center justify-between gap-3">
 <span className="rounded-full bg-primary/[0.15] px-3 py-1 text-xs font-black text-[#7c2d12]">Etape {index + 1}</span>
 ...
 <p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-[hsl(var(--primary))]">{ownerLabels[step.owner]}</p>
 ...
 <section className="mx-auto max-w-7xl px-4 py-14">
 <div className="rounded-[2rem] border border-primary/20 bg-white p-6 shadow-elegant lg:p-8">
 <SectionHeading eyebrow="Prochaine execution" title="Ce qui doit etre prouve sur le terrain." />
 <div className="mt-6 grid gap-4 md:grid-cols-3">
 {(readiness?.nextActions ?? ['Finaliser le test client, vendeur, livreur et preuve de remise.','Mesurer le delai de qualification et la marge par commande.','Qualifier les documents partenaires avant visibilite publique.',
 ]).map((action) => (
 <div key={action} className="rounded-[1.35rem] border border-primary/20 bg-[#fffaf4] p-5">
 <BadgeCheck className="h-6 w-6 text-[hsl(var(--primary))]" />
 <p className="mt-4 text-sm font-bold leading-6 text-stone-700">{action}</p>
 </div>
 ))}
 </div>
 <div className="mt-7 flex flex-col gap-3 sm:flex-row">
 <a href={`${baseUrl}#partenaires`} className="inline-flex items-center justify-center gap-2 rounded-full bg-[hsl(var(--primary))] px-6 py-4 text-sm font-black uppercase tracking-[0.14em] text-white">
 Ajouter partenaires <Store className="h-4 w-4" />
 </a>
 <a href={`${baseUrl}?view=admin-documents`} className="inline-flex items-center justify-center gap-2 rounded-full border border-primary/20 bg-white px-6 py-4 text-sm font-black text-[#7c2d12]">
...
function SectionHeading({ eyebrow, title }: { eyebrow: string; title: string }) {
 return (
 <div>
 <p className="text-xs font-black uppercase tracking-[0.22em] text-[hsl(var(--primary))]">{eyebrow}</p>
...
function MetricCard({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) {
 return (
 <div className="rounded-2xl border border-primary/20 bg-[#fffaf4] p-4">
