...
export default function AdminDashboardPage() {
 ...
 <div className="bg-muted border border-input rounded-xl p-4 mb-8">
 <p className="font-bold text-muted-foreground mb-2">🔔 Actions à faire</p>
 {newSubmissions > 0 && <p className="text-sm text-muted-foreground">• {newSubmissions} correction(s) partenaire à traiter</p>}
 {unreadMessages > 0 && <p className="text-sm text-muted-foreground">• {unreadMessages} message(s) non lu(s)</p>}
 {partnersOpened.length < 3 && <p className="text-sm text-muted-foreground">• {3 - partnersOpened.length} partenaire(s) n'ont pas encore ouvert leur accès</p>}
 </div>
 )}
 ...
 <div key={i} className="text-sm p-3 bg-muted rounded-xl">
 <p className="font-semibold">{s.partnerName} <span className={`text-xs px-2 py-0.5 rounded-full ml-2 ${s.status ==='pending' ?'bg-muted text-muted-foreground' :'bg-green-100 text-green-700'}`}>{s.status}</span></p>
