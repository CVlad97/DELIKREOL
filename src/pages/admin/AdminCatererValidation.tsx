...
export default function AdminCatererValidation() {
 ...
 app.status ==='integrated' ?'border-green-300' :'border-border'
 }`}
 >
 {/* Main info row */}
 ...
 <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${STATUS_CONFIG[app.status]?.class ||'bg-muted text-foreground border-input'}`}>
 {STATUS_CONFIG[app.status]?.label || app.status}
 </span>
 </div>
