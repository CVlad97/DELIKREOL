...
export function AdminView() {
 ...
 <div className="text-[10px] text-success font-bold">Commission: {platformCommission.toFixed(0)} EUR</div>
 </div>
 <div className="bg-card rounded-2xl border border-border/50 p-4">
 <div className="flex items-center gap-2 mb-2">
 ...
 <div className={`text-[10px] font-bold ${saturatedRelays.length > 0 ?'text-amber-600' :'text-success'}`}>
 ...
 const { pct } = getRelayLoad(r.id);
 return (
 <div key={r.id} className="p-3 bg-muted border border-input rounded-xl flex items-center gap-3">
 <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0" />
 <div>
 <p className="text-sm font-bold text-muted-foreground">{r.name} - {pct}%</p>
 ...
 <span className={`w-2 h-2 rounded-full ${v.active ?'bg-success/[0.15]0' :'bg-red-400'}`} />
 </div>
 </div>
 );
 })}
 ...
 <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${pct > 80 ?'bg-red-100 text-red-700' : pct > 50 ?'bg-muted text-muted-foreground' :'bg-success/[0.15] text-success'}`}>
 {pct}%
 </span>
 </div>
