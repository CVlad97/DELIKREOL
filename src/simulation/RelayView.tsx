...
export function RelayView() {
 ...
 <span className={`font-black ${isFull ?'text-red-600' : isHigh ?'text-amber-600' :'text-success'}`}>{pct}%</span>
 </div>
 <div className="h-2.5 bg-muted rounded-full overflow-hidden">
 <div
 className={`h-full rounded-full transition-all ${isFull ?'bg-red-500' : isHigh ?'bg-muted0' :'bg-success/[0.15]0'}`}
 style={{ width: `${pct}%` }}
 />
 </div>
 </div>
 ...
 <div key={o.id} className="bg-card rounded-2xl border border-input p-4 space-y-2">
 <div className="flex items-center justify-between">
 <span className="font-bold text-foreground text-sm">{o.orderNumber}</span>
 <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-muted text-muted-foreground border border-input">
 ...
 <div className="space-y-3">
 {waitingOrders.map(o => (
 <div key={o.id} className="bg-card rounded-2xl border border-success/20 p-4 space-y-2">
 <div className="flex items-center justify-between">
 <span className="font-bold text-foreground text-sm">{o.orderNumber}</span>
 <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-success/[0.15] text-success border border-success/20">
 Au relais
 </span>
 </div>
 <div className="text-xs text-muted-foreground">
