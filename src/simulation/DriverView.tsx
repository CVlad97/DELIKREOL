...
export function DriverView() {
 ...
 <div key={n.id} className={`flex items-center gap-3 p-3 rounded-xl border text-sm ${n.type ==='warning' ?'bg-muted border-input text-muted-foreground' : n.type ==='success' ?'bg-success/[0.15] border-success/20 text-success' :'bg-sky-50 border-sky-200 text-sky-800'}`}>
 ...
 {o.requiresColdChain && (
 <div className={`flex items-center gap-2 p-2 rounded-lg ${coldWarning ?'bg-muted border border-input' :'bg-sky-50 border border-sky-200'}`}>
 {coldWarning ? <AlertTriangle className="w-4 h-4 text-amber-600" /> : <Snowflake className="w-4 h-4 text-sky-500" />}
 <span className={`text-xs font-bold ${coldWarning ?'text-muted-foreground' :'text-sky-700'}`}>
 ...
 className={`px-5 py-2.5 rounded-xl font-bold text-sm active:scale-95 transition-all ${coldWarning ?'bg-muted0 text-white' :'bg-primary text-primary-foreground'}`}
 >
 ...
 <div className={`w-3 h-3 rounded-full ${isPickedUp ?'bg-success/[0.15]0' :'bg-primary animate-pulse'}`} />
 ...
 target="_blank"
 rel="noopener noreferrer"
 className="flex-1 py-2 bg-success/[0.15]0 text-white rounded-lg font-bold text-xs text-center flex items-center justify-center gap-1"
 >
 ...
 className="flex-1 py-3 bg-success/[0.15]0 text-white rounded-xl font-bold text-sm active:scale-95 transition-all flex items-center justify-center gap-2"
 >
 ...
 <span className="font-black text-sm text-success">+{(o.deliveryFee * 0.7).toFixed(2)} EUR</span>
 </div>
 ))}
 </div>
 </div>
